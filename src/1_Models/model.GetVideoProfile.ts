...
export const _getVideoProfile = async (user_id: number, profile_id: number) => {
  try {
    const [selectedVideoProfile] = await db('video_metadata')
      .select('profile_name','codec_name', 'width', 'height', 'field_order', 'r_frame_rate', 'duration', 'bitrate', 'audio_codec_name', 'sample_rate', 'channels', 'channel_layout', 'audio_bitrate')
